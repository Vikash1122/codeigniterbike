...
function initMap(vno='') {
  //Map location
  var MapLocation;
  var mapObject = new google.maps.Map(document.getElementById("map-with-marker"));
  var MapZoom = 12;
   localStorage.Zoom=12;
  
  // setInterval(function () {
   

     $.get("/swagbike/admin/getbikelocation?vehicle_no="+vno+"", function(data, status){
 var datas=$.parseJSON(data)
 MapLocation=datas.data;
if(datas.status==='false')
{
  $('#map-with-marker').html('<div class="alert alert-danger"><strong>Alert! </strong>  Sorry!, No location found of this vehicle_no</div>');
  return false;
}
 console.log(status);
  ...
  if($("#map-with-marker").length) {
     ...
     var map = new google.maps.Map(document.getElementById('map-with-marker'), {
      ...
      marker = new google.maps.Marker({
        ...
        map: map,
        icon: '/swagbike/admin/images/flag.png'
      });

      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          infowindow.setContent(MapLocation[i].vehicle_no,MapLocation[i].status);
