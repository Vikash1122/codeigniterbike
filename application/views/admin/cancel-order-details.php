<?php 
$this->db->select("cancel_bike_orders.id as order_id,bike_details.name as bike_name,bike_details.engine,bike_details.rent_type,concat_ws(' ',users.user_firstname,users.user_lastname) as user_name,DATE_FORMAT(cancel_bike_orders.booking_date,'%d/%m/%Y %h:%i %p') as booking_date,DATE_FORMAT(cancel_bike_orders.cancel_date,'%d/%m/%Y %h:%i %p') as cancel_date,image_name,concat('".base_url()."uploads/bikes/',bike_images.image_name) as image_url,users.user_image,concat('".base_url()."uploads/users/',users.user_image) as image_url1,users.user_id,bike_details.hourly_price,bike_details.category,bike_details.vehicle_no,cancel_bike_orders.reason")
       ->from("cancel_bike_orders")
       ->join("bike_details","bike_details.id=cancel_bike_orders.bike_id","left")
       ->join("bike_images","bike_images.bike_id=cancel_bike_orders.bike_id","left")
       ->join("users","cancel_bike_orders.user_id=users.user_id","left");     
      $res=$this->db->where('cancel_bike_orders.id',$_GET['order_id'])->get()->row_array();
                ...
                <div class="content-wrapper">
                    <div class="row">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="py-4">
                                        ...
                                        <p class="clearfix">
                                            <span class="float-left">Cancel Time</span>
                                            <span class="float-right text-muted"><?=$res['cancel_date']?></span>
                                        </p>
                                         <p class="clearfix">
                                            <span class="float-left">Cancel Reason</span>
                                            <span class="float-right text-muted"><?=$res['reason']?></span>
                                        </p>
                                        <p class="clearfix">
                        ...
                        <div class="col-md-8 grid-margin grid-margin-md-0 ">
							<div class="card">
								<div class="card-body">
                                    <h4 class="card-title">Order ID : <strong>cord_00<?=$_GET['order_id']?></strong></h4>
